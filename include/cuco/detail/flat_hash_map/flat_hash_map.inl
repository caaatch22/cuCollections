  ...
  InputIt first, InputIt last, Hash hash, KeyEqual key_equal, cudaStream_t stream)
{
  ...
  auto counter = detail::counter_storage<size_t, Scope, Allocator>{this->allocator_};
  counter.reset({stream});

  detail::insert<block_size, tile_size>
    <<<grid_size, block_size, 0, stream>>>(first, num_keys, counter.data(), view, hash, key_equal);
  auto const h_num_successes = counter.load_to_host({stream});
