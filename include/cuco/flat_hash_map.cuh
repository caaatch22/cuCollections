...
class flat_hash_map {
 ...
 public:
  ...
  using ref_type = cuco::flat_hash_map_ref<key_type,
                                           mapped_type,
                                           thread_scope,
                                           key_equal,
                                           probing_scheme_type,
  ...
  flat_hash_map(flat_hash_map&&) = default;  ///< Move constructor
   ...
   */
  constexpr flat_hash_map(Extent capacity,
   ...
   * @param stream CUDA stream used to initialize the map
   */
  constexpr flat_hash_map(Extent n,
   ...
   * flat_hash_map<K, V>::value_type></tt> is `true`
   *
   ...
   *
   * @return Device ref of the current `flat_hash_map` object
   */
  template <typename... Operators>
  [[nodiscard]] auto ref(Operators... ops) const noexcept;

 private:
  std::unique_ptr<impl_type> impl_;   ///< Flat hash map implementation
...
#include <cuco/detail/flat_hash_map/flat_hash_map.inl>
