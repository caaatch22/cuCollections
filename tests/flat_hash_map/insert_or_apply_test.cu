...
void test_insert_or_apply_shmem(Map& map, size_type num_keys, size_type num_unique_keys, Init init)
{
  ...
  using shared_map_type = cuco::experimental::flat_hash_map<Key,
                                                            Value,
                                                            extent_type,
...
TEMPLATE_TEST_CASE_SIG(
  "flat_hash_map insert_or_apply tests",
  "",
  ...
  (int64_t, int64_t, cuco::test::probe_sequence::linear_probing, 2))
{
  ...
  using map_type = cuco::experimental::flat_hash_map<Key,
                                    Value,
...
TEMPLATE_TEST_CASE_SIG(
  "flat_hash_map insert_or_apply all unique keys tests", "", ((typename Key)), (int32_t), (int64_t))
{
  using Value = Key;
...
TEMPLATE_TEST_CASE_SIG(
  "flat_hash_map insert_or_apply shared memory", "", ((typename Key)), (int32_t), (int64_t))
{
  using Value = Key;

  using map_type =
    cuco::experimental::flat_hash_map<Key,
                                      Value,
                                      cuco::extent<size_type>,
                                      cuda::thread_scope_device,
                                      cuda::std::equal_to<Key>,
