...
TEMPLATE_TEST_CASE_SIG("flat_hash_map find tests",
                       "",
                       ...
                       (int64_t, int64_t))
{
  ...
  cuco::flat_hash_map<Key, Value> indentity_hash_map{num_keys,
                                                     cuco::empty_key<Key>{-1},
                                                     cuco::empty_value<Value>{-1},
                                                     cuco::erased_key<Key>{-2},
                                                     default_load_factor};
  ...
  SECTION("Check find in a all erased submap")
  {
    ...
    REQUIRE(indentity_hash_map.size() == first_insert_size);

    indentity_hash_map.erase(
      d_keys.begin(), d_keys.begin() + first_insert_size, cuco::identity_hash<Key>());
    REQUIRE(indentity_hash_map.size() == 0);

    indentity_hash_map.insert(
      pairs_begin + first_insert_size, pairs_begin + num_keys, cuco::identity_hash<Key>());
    REQUIRE(indentity_hash_map.size() == num_keys - first_insert_size);

    indentity_hash_map.erase(
      d_keys.begin() + first_insert_size, d_keys.end(), cuco::identity_hash<Key>());
    REQUIRE(indentity_hash_map.size() == 0);

    // we've construct a flat_hash_map with one submap whose keys are all erased keys (-2 in this
