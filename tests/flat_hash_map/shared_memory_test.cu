...
TEMPLATE_TEST_CASE_SIG("flat_hash_map shared memory tests",
                       "",
                       ...
                       (int64_t, int64_t))
{
  ...
  // using std::unique_ptr because flat_hash_map does not have copy/move constructor/assignment
  // operator yet
...
__global__ void shared_memory_hash_table_kernel(bool* key_found)
{
  ...
  auto raw_ref = cuco::flat_hash_map_ref{
